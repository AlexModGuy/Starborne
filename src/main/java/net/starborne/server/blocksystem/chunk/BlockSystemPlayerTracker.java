package net.starborne.server.blocksystem.chunk;
...
public class BlockSystemPlayerTracker {
    ...
    public void removePlayer(EntityPlayerMP player) {
        if (this.players.contains(player)) {
            ...
            if (this.sentToPlayers) {
                Starborne.NETWORK_WRAPPER.sendTo(new UnloadChunkMessage(this.blockSystem, this.chunkPosition.chunkXPos, this.chunkPosition.chunkZPos), player);
            }
            this.players.remove(player);
    ...
    public boolean sendToPlayers() {
        if (this.sentToPlayers) {
            ...
            for (EntityPlayerMP player : this.players) {
                Starborne.NETWORK_WRAPPER.sendTo(message, player);
//                this.blockSystem.getEntityTracker().sendLeashedEntitiesInChunk(player, this.providingChunk); TODO
    ...
    public void sendNearbySpecialEntities(EntityPlayerMP player) {
        if (this.sentToPlayers) {
            Starborne.NETWORK_WRAPPER.sendTo(new ChunkMessage(this.blockSystem, this.providingChunk, 0xFFFF), player);
